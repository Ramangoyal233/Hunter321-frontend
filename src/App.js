...
const AppContent = () => {
  ...
  const { user, isAuthenticated, loading, logout, setUser, setIsAdmin } = useAuth();
    ...
    <div className="min-h-screen bg-gray-900 text-white relative">
      ...
      <div className="relative z-20 min-h-screen">
        ...
        <div className="fixed top-0 left-1/2 transform -translate-x-1/2 z-50 w-[90%] sm:w-[80%] max-w-5xl mt-4">
          <div 
            className="transition-all duration-300 rounded-[2rem] px-4 sm:px-8 py-2.5 flex items-center gap-4 border-2"
            style={getHeaderStyle()}
          >
            {/* Logo Section */}
            <Link to={user?.role === 'admin' ? "/admin" : "/"} className="flex items-center group pl-[13px]">
              <span className={`text-sm sm:text-base font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent group-hover:from-blue-500 group-hover:to-purple-600 transition-all duration-300 whitespace-nowrap ${
                isScrolled ? 'text-shadow-glow' : ''
              }`}>
                BugBounty Hub
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-4">
              {getNavLinks().map((item) => (
                <Link 
                  key={item.name}
                  to={item.path}
                  className={`text-gray-300 hover:text-white transition-colors duration-300 text-sm font-medium tracking-wide ${isScrolled ? 'hover:text-blue-400' : ''}`}
                >
                  {item.name}
                </Link>
              ))}
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="hidden md:flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-sm font-medium text-red-400 hover:text-red-300 transition-colors duration-300 border border-red-400/20 hover:border-red-400/40 rounded-lg"
                  >
                    Logout
                  </button>
                </div>
              ...
              <>
                ...
                <div className="hidden md:flex items-center gap-4">
                  <Link
                    to="/signin"
                    className="px-4 py-1.5 text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors duration-300 border border-blue-400/20 hover:border-blue-400/40 rounded-lg"
                  >
                    Login
                  </Link>
                </div>
            ...
            <button
              ...
              className="md:hidden p-2 pl-[131px] text-gray-300 hover:text-white focus:outline-none"
          ...
          {/* Mobile Menu */}
          <div
            className={`md:hidden absolute top-full left-0 right-0 mt-2 rounded-2xl overflow-hidden transition-all duration-300 transform ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}
            style={getHeaderStyle()}
          >
            <div className="p-4 space-y-4">
              ...
              {getNavLinks().map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="block text-gray-300 hover:text-white transition-colors duration-300 text-base font-medium tracking-wide py-2"
              ...
              {isAuthenticated && (
                <div className="pt-4 border-t border-gray-700">
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-red-400 hover:text-red-300 transition-colors duration-300 border border-red-400/20 hover:border-red-400/40 rounded-lg"
                  >
                    Logout
                  </button>
                </div>
