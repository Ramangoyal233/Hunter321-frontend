            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  <span className="flex items-center gap-0.5 text-blue-400 font-medium bg-blue-500/10 px-2 py-0.5 rounded-full text-xs">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    {writeup.reads || 0} reads
                  </span>
                  {writeup.bounty?.amount > 0 && (
                    <span className="text-blue-400 font-medium flex items-center gap-0.5 bg-blue-500/10 px-2 py-0.5 rounded-full text-xs">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      ${writeup.bounty.amount}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-zinc-400">{writeup.author?.username}</span>
                </div>
                <Link
                  to={`/writeups/${writeup._id}`}
                  className="text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-0.5 text-sm"
                >
                  Read More
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
